...
/**
 * FO-14 Onboarding state machine — complete flow
 *
 * Discovery (steps 0-5):
...
export function FOExperience() {
  ...
  const handleGoalContinue = useCallback(() => {
    const goalQuestion = FO14_GOAL_QUESTION.replace('[name]', state.name);
    const goalExchange = {
    ...
    const context: FO14OnboardingData = {
      name: state.name,
    ...
    generateDiscoveryStep(4, context, implementation).then((result) => {
      ...
      if (result.skip) {
        ...
        generateDiscoveryStep(5, context, implementation).then((step5Result) => {
          ...
          console.error('[fo-14] Error fetching step 5 (after skip):', error);
          setState((prev) => ({
            ...prev,
            isLoadingDiscovery: false,
      ...
      console.error('[fo-14] Error fetching step 4:', error);
      setState((prev) => ({
        ...prev,
        isLoadingDiscovery: false,
        discoveryError: 'An unexpected error occurred',
  ...
  const handleContextContinue = useCallback(() => {
    ...
    generateDiscoveryStep(5, context, implementation).then((result) => {
      ...
      console.error('[fo-14] Error fetching step 5:', error);
      setState((prev) => ({
        ...prev,
        isLoadingDiscovery: false,
        discoveryError: 'An unexpected error occurred',
  ...
  const handleToneContinue = useCallback(() => {
    ...
    generateAffirmationBatchFO14({
      ...
      console.error('[fo-14] Error generating first batch:', error);
      setState((prev) => ({
        ...prev,
        isGenerating: false,
  ...
  const handleBatchComplete = useCallback((loved: string[], discarded: string[]) => {
    ...
    if (step >= 8 && step <= 10) {
      ...
      generateAffirmationBatchFO14({
        ...
        console.error('[fo-14] Error generating batch:', error);
        setState((prev) => ({
          ...prev,
          isGenerating: false,
          generationError: 'An unexpected error occurred',
  ...
  const handleCreateListContinue = useCallback(() => {
    ...
    generateAffirmationBatchFO14({
      ...
      console.error('[fo-14] Error generating Phase 2 batch:', error);
      setState((prev) => ({
        ...prev,
        isGenerating: false,
        generationError: 'An unexpected error occurred',
