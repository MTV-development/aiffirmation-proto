...
import { createFO02Agent } from '@/src/mastra/agents/fo-02';
import { renderTemplate, getKVText } from '@/src/services';

export interface FO02GenerateBatchOptions {
  name: string;
  intention: string;
  batchNumber: number;
...
export interface FO02GenerateBatchResult {
  affirmations: string[];
  error?: string;
}
...
/**
 * Server action to generate a batch of 10 affirmations using FO-02 Mastra agent.
 * Supports iterative improvement through feedback from previous batches.
 */
export async function generateAffirmationBatchFO02(
  options: FO02GenerateBatchOptions
): Promise<FO02GenerateBatchResult> {
  const {
    name,
    intention,
    batchNumber,
  ...
  try {
    ...
    const temperatureKey = `versions.fo-02._temperature.${implementation}`;
    ...
    console.log('[fo-02] Implementation:', implementation);
    console.log('[fo-02] Batch number:', batchNumber);
    console.log('[fo-02] Temperature:', temperature);
    console.log('[fo-02] Approved count:', approvedAffirmations.length);
    console.log('[fo-02] Skipped count:', skippedAffirmations.length);
    console.log('[fo-02] User prompt:', userPrompt);
    ...
    console.log('[fo-02] Response length:', result.text.length);

    // Parse JSON array response
    const affirmations = parseAffirmationsResponse(result.text);

    if (affirmations.length === 0) {
      console.error('[fo-02] Failed to parse affirmations from response:', result.text.substring(0, 500));
      return {
        affirmations: [],
    ...
    console.log('[fo-02] Parsed affirmations count:', affirmations.length);

    return { affirmations };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    console.error('[fo-02] Error generating affirmations:', errorMessage);
    return {
      affirmations: [],
      error: errorMessage,
    };
