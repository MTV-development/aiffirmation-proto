...
import { createFO13DiscoveryAgent } from '@/src/mastra/agents/fo-13/discovery-agent';
import { createFO13AffirmationAgent } from '@/src/mastra/agents/fo-13/affirmation-agent';
import { renderTemplate } from '@/src/services';
import {
  type FO13OnboardingData,
  type FO13DiscoveryResponse,
  type FO13GenerateBatchOptions,
  type FO13GenerateBatchResult,
  PHASE1_BATCH_SIZE,
} from './types';
 ...
 */
function parseDiscoveryResponse(text: string): FO13DiscoveryResponse | null {
...
async function buildDiscoveryPrompt(
  ...
  try {
    ...
    console.warn('[fo-13-discovery] Template not found, using fallback:', error);
    const lines: string[] = [];
...
export async function generateDiscoveryStep(
  ...
  try {
    ...
    const agent = await createFO13DiscoveryAgent(implementation);

    console.log('[fo-13-discovery] Step number:', stepNumber);
    console.log('[fo-13-discovery] Name:', context.name);
    console.log('[fo-13-discovery] Exchanges count:', context.exchanges.length);
    console.log('[fo-13-discovery] User prompt:', userPrompt);

    // Generate with Mastra agent
    const result = await agent.generate(userPrompt);

    console.log('[fo-13-discovery] Response length:', result.text.length);
    console.log('[fo-13-discovery] Full response:', result.text);

    // Parse JSON response
    const response = parseDiscoveryResponse(result.text);

    if (!response) {
      console.error('[fo-13-discovery] Failed to parse discovery response:', result.text.substring(0, 500));
      return {
        skip: false,
        question: '',
        initialChips: [],
    ...
    if (stepNumber === 5 && response.skip) {
      console.warn('[fo-13-discovery] Agent returned skip=true for step 5 — forcing skip=false');
      response.skip = false;
    }

    console.log('[fo-13-discovery] Parsed successfully');
    console.log('[fo-13-discovery] Skip:', response.skip);
    console.log('[fo-13-discovery] Question:', response.question);
    console.log('[fo-13-discovery] Initial chips count:', response.initialChips.length);
    console.log('[fo-13-discovery] Expanded chips count:', response.expandedChips.length);

    return response;
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    console.error('[fo-13-discovery] Error generating discovery step:', errorMessage);
    return {
      skip: false,
      question: '',
      initialChips: [],
...
/**
 * Build the user prompt for affirmation generation from FO13OnboardingData.
...
async function buildAffirmationPrompt(
  ...
  try {
    ...
    // Fallback to hardcoded prompt if template not found
    console.warn('[fo-13-affirmations] Template not found, using fallback:', error);
    const lines: string[] = [];
...
/**
 * Server action to generate a batch of affirmations using the FO-13 affirmation agent.
 ...
 */
export async function generateAffirmationBatchFO13(
  options: FO13GenerateBatchOptions
): Promise<FO13GenerateBatchResult> {
  const {
    context,
    batchNumber,
  ...
  try {
    ...
    // Use FO-13 affirmation agent
    const agent = await createFO13AffirmationAgent(implementation);

    console.log('[fo-13-affirmations] Batch number:', batchNumber);
    console.log('[fo-13-affirmations] Batch size:', effectiveBatchSize);
    console.log('[fo-13-affirmations] Exchanges count:', context.exchanges.length);
    console.log('[fo-13-affirmations] Approved count:', approvedAffirmations.length);
    console.log('[fo-13-affirmations] Skipped count:', skippedAffirmations.length);
    console.log('[fo-13-affirmations] User prompt:', userPrompt);
    ...
    console.log('[fo-13-affirmations] Response length:', result.text.length);
    console.log('[fo-13-affirmations] Full response:', result.text);
    ...
    if (affirmations.length === 0) {
      console.error(
        '[fo-13-affirmations] Failed to parse affirmations from response:',
    ...
    console.log('[fo-13-affirmations] Parsed affirmations count:', affirmations.length);

    return { affirmations };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    console.error('[fo-13-affirmations] Error generating affirmations:', errorMessage);
    return {
      affirmations: [],
      error: errorMessage,
    };
