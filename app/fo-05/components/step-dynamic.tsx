...
interface StepDynamicProps {
  ...
  onAnswer: (question: string, answer: { text: string; selectedFragments: string[] }) => void;
  onReadyForAffirmations: () => void;
  onError: (error: string) => void;
}

/**
 * Dynamic detail-gathering step component for FO-05 onboarding.
 *
 * This component:
 * 1. Shows 'Thinking...' loading state while fetching next screen
 * 2. Calls generateDynamicScreen server action
 * 3. Renders FragmentInput with agent response
...
export function StepDynamic({
  ...
  const handleContinue = async () => {
    ...
    // Note: FragmentInput appends fragments to text, so we pass empty selectedFragments
    // The fragments are embedded in the text itself
    onAnswer(screenData.question, { text: inputValue.text, selectedFragments: [] });
