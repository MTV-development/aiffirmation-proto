...
import { createFO08DiscoveryAgent } from '@/src/mastra/agents/fo-08/agent';
import { createFO08AffirmationAgent } from '@/src/mastra/agents/fo-08/affirmation-agent';
import { createFO08SummaryAgent } from '@/src/mastra/agents/fo-08/summary-agent';
...
import {
  ...
  type FO08GenerateBatchOptions,
  type FO08GenerateBatchResult,
} from './types';
 ...
 * FO-08 has optional reflectiveStatement field.
 */
function isValidDynamicScreenResponse(obj: unknown): obj is DynamicScreenResponse {
  ...
  // reflectiveStatement is optional, but when present must be a string
  if (response.reflectiveStatement !== undefined && typeof response.reflectiveStatement !== 'string') {
    return false;
  }
...
async function buildFirstScreenPrompt(name: string, implementation: string): Promise<string> {
  try {
    ...
    console.warn('[fo-08] Template not found, using fallback:', error);
    return `## User Context
Name: ${name}
...
export async function generateFirstScreenFragments(
  ...
  try {
    ...
    // Create agent
    const agent = await createFO08DiscoveryAgent(implementation);

    console.log('[fo-08] Generating first screen fragments for:', name);
    console.log('[fo-08] User prompt:', userPrompt);

    // Generate with Mastra agent
    const result = await agent.generate(userPrompt);

    console.log('[fo-08] Response length:', result.text.length);
    console.log('[fo-08] Raw response:', result.text.substring(0, 500));

    // Parse JSON response
    const response = parseFirstScreenFragments(result.text);

    if (!response) {
      console.error('[fo-08] Failed to parse first screen response:', result.text.substring(0, 500));
      return {
        initialFragments: [],
        expandedFragments: [],
    ...
    console.log('[fo-08] Parsed first screen fragments successfully');

    return response;
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    console.error('[fo-08] Error generating first screen fragments:', errorMessage);
    return {
      initialFragments: [],
      expandedFragments: [],
      error: errorMessage,
...
/**
 * Server action to generate a dynamic discovery screen using the FO-08 agent.
...
export async function generateDynamicScreen(
  ...
  try {
    ...
    console.log('[fo-08] Screen number:', context.screenNumber);
    console.log('[fo-08] Exchanges count:', context.exchanges.length);
    console.log('[fo-08] User prompt:', userPrompt);
    ...
    if (!response) {
      console.error('[fo-08] Failed to parse response:', result.text.substring(0, 500));
      return {
        question: '',
        initialFragments: [],
        expandedFragments: [],
    ...
    console.log('[fo-08] Parsed successfully');
    console.log('[fo-08] readyForAffirmations:', response.readyForAffirmations);

    return response;
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    console.error('[fo-08] Error generating dynamic screen:', errorMessage);
    return {
      question: '',
      initialFragments: [],
      expandedFragments: [],
 ...
 * FO-08 simplifies: no familiarity, no initialTopic, simpler answer structure.
 */
function buildAffirmationPrompt(
  ...
  // User profile section - simplified for FO-08
...
/**
 * Server action to generate a batch of 10 affirmations using the FO-08 agent.
 */
export async function generateAffirmationBatchFO08(
  options: FO08GenerateBatchOptions
): Promise<FO08GenerateBatchResult> {
  const {
    context,
    batchNumber,
  ...
  try {
    ...
    // Use FO-08 affirmation agent
    const agent = await createFO08AffirmationAgent(implementation);

    console.log('[fo-08-affirmations] Implementation:', implementation);
    console.log('[fo-08-affirmations] Batch number:', batchNumber);
    console.log('[fo-08-affirmations] Exchanges count:', context.exchanges.length);
    console.log('[fo-08-affirmations] Approved count:', approvedAffirmations.length);
    console.log('[fo-08-affirmations] Skipped count:', skippedAffirmations.length);
    console.log('[fo-08-affirmations] User prompt:', userPrompt);
    ...
    console.log('[fo-08-affirmations] Response length:', result.text.length);
    ...
    if (affirmations.length === 0) {
      console.error(
        '[fo-08-affirmations] Failed to parse affirmations from response:',
    ...
    console.log('[fo-08-affirmations] Parsed affirmations count:', affirmations.length);

    return { affirmations };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    console.error('[fo-08-affirmations] Error generating affirmations:', errorMessage);
    return {
      affirmations: [],
      error: errorMessage,
    };
 ...
 * FO-08 simplifies: no familiarity, no initialTopic.
 */
...
/**
 * Server action to generate a summary for the review screen.
 * Uses a single summary agent (like FO-07), shown after affirmations are generated.
...
export async function generateSummary(
  ...
  try {
    ...
    const agent = await createFO08SummaryAgent(implementation);

    console.log('[fo-08-summary] Implementation:', implementation);
    console.log('[fo-08-summary] Name:', context.name);
    console.log('[fo-08-summary] Exchanges count:', context.exchanges.length);
    console.log('[fo-08-summary] Affirmation types:', affirmationTypes.length);

    // Generate the summary
    const result = await agent.generate(userPrompt);

    console.log('[fo-08-summary] Response length:', result.text.length);
    console.log('[fo-08-summary] Summary:', result.text.substring(0, 200));

    // Return the summary text directly (agent returns plain text)
    return result.text.trim();
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    console.error('[fo-08-summary] Error generating summary:', errorMessage);
