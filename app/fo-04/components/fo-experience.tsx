...
/**
 * FO-04 Onboarding data collected during the flow.
 * Key difference from FO-03: uses GatheringContext with dynamic exchanges
 * instead of hardcoded situation/feelings/whatHelps fields.
 */
export interface FO04OnboardingData {
  name: string;
...
/**
 * FO-04 Onboarding state
 *
 * Flow phases:
 * - Steps 0-2: Welcome screens (same as FO-03)
 ...
 * - Steps 5-7 (hardcoded situation/feelings/whatHelps) are replaced by
 *   dynamic AI-generated discovery screens
 * - Uses GatheringContext to accumulate exchange history
 */
export interface OnboardingState extends FO04OnboardingData {
...
/**
 * Main state manager component for FO-04 onboarding experience.
 *
 * Responsibilities:
 ...
 *
 * Key differences from FO-03:
 * - Dynamic discovery phase replaces hardcoded steps 5-7
 * - Uses GatheringContext to track exchange history for AI context
 * - AI determines when enough context is gathered (2-5 screens)
 */
export function FOExperience() {
  ...
  const handleDynamicAnswer = useCallback(
    (answer: { text: string; selectedChips: string[] }) => {
      setState((prev) => {
        // Get the current question from the last screen shown
        // The question will be stored when we transition
        const lastExchange = prev.gatheringContext.exchanges[prev.gatheringContext.exchanges.length - 1];

        // If we already have the exchange recorded with a question, update it with the answer
        // Otherwise, we need to add a new exchange (this happens on continue)
        const newExchanges = [...prev.gatheringContext.exchanges];

        // The question should have been set when the dynamic screen loaded
        // For now, we store the answer and let the screen number increment
        // The actual question text will be captured from the DynamicScreenResponse
        ...
        return {
          ...
          gatheringContext: {
            ...
            exchanges: [
              ...
              {
                question: '', // Will be filled by step-dynamic's screenData
                answer,
              },
            ],
  ...
  // Handle storing the question from step-dynamic before answer
  ...
  const handleDynamicError = useCallback((error: string) => {
    console.error('[fo-04] Dynamic screen error:', error);
  ...
  const generateBatch = useCallback(
    async (batchNumber: 1 | 2 | 3) => {
      ...
      try {
        const result = await generateAffirmationBatchFO04({
