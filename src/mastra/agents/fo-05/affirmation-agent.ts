...
export const fo05AffirmationAgent = new Agent({
  id: 'fo-05-affirmation-agent',
  name: 'FO-05 Affirmation',
  instructions: DEFAULT_INSTRUCTIONS,
  model: getModel(),
});

export async function createFO05AffirmationAgent(implementation: string = 'default'): Promise<Agent> {
  const systemPrompt = await getAgentSystemPrompt('fo-05-affirmation', implementation);
  const modelName = await getAgentModelName('fo-05-affirmation', implementation);

  return new Agent({
    id: `fo-05-affirmation-agent-${implementation}`,
    name: 'FO-05 Affirmation',
