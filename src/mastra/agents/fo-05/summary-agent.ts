...
export const fo05PreSummaryAgent = new Agent({
  id: 'fo-05-pre-summary',
  name: 'FO-05 Pre-Affirmation Summary',
  instructions: PRE_AFFIRMATION_INSTRUCTIONS,
  model: getModel(),
});

// Post-affirmation agent (after generation)
export const fo05PostSummaryAgent = new Agent({
  id: 'fo-05-post-summary',
  name: 'FO-05 Post-Affirmation Summary',
  instructions: POST_AFFIRMATION_INSTRUCTIONS,
  model: getModel(),
});

export async function createFO05PreSummaryAgent(implementation: string = 'default'): Promise<Agent> {
  const systemPrompt = await getAgentSystemPrompt('fo-05-pre-summary', implementation);
  const modelName = await getAgentModelName('fo-05-pre-summary', implementation);

  return new Agent({
    id: `fo-05-pre-summary-${implementation}`,
    name: 'FO-05 Pre-Affirmation Summary',
...
export async function createFO05PostSummaryAgent(implementation: string = 'default'): Promise<Agent> {
  const systemPrompt = await getAgentSystemPrompt('fo-05-post-summary', implementation);
  const modelName = await getAgentModelName('fo-05-post-summary', implementation);

  return new Agent({
    id: `fo-05-post-summary-${implementation}`,
    name: 'FO-05 Post-Affirmation Summary',
...
export const fo05SummaryAgent = fo05PostSummaryAgent;
export const createFO05SummaryAgent = createFO05PostSummaryAgent;
