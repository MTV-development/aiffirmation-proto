...
export const fo10AffirmationAgent = new Agent({
  id: 'fo-10-affirmation',
  name: 'FO-10 Affirmation',
  instructions: DEFAULT_INSTRUCTIONS,
  model: getModel(),
});

export async function createFO10AffirmationAgent(implementation: string = 'default'): Promise<Agent> {
  const systemPrompt = await getAgentSystemPrompt('fo-10-affirmation', implementation);
  const modelName = await getAgentModelName('fo-10-affirmation', implementation);

  return new Agent({
    id: `fo-10-affirmation-${implementation}`,
    name: 'FO-10 Affirmation',
