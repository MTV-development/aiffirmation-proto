...
export const fo14DiscoveryAgent = new Agent({
  id: 'fo-14-discovery',
  name: 'FO-14 Discovery',
  instructions: DEFAULT_INSTRUCTIONS,
  model: getModel('openai/gpt-4o'),
});

export async function createFO14DiscoveryAgent(implementation: string = 'default'): Promise<Agent> {
  const systemPrompt = await getAgentSystemPrompt('fo-14', implementation);
  const modelName = await getAgentModelName('fo-14', implementation);

  return new Agent({
    id: `fo-14-discovery-${implementation}`,
    name: 'FO-14 Discovery',
