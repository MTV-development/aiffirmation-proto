...
export const fo14AffirmationAgent = new Agent({
  id: 'fo-14-affirmation',
  name: 'FO-14 Affirmation',
  instructions: DEFAULT_INSTRUCTIONS,
  model: getModel(),
});

export async function createFO14AffirmationAgent(implementation: string = 'default'): Promise<Agent> {
  const systemPrompt = await getKVText(`versions.fo-14.system_affirmation.${implementation}`);
  const modelName = await getKVText(`versions.fo-14._model_name_affirmation.${implementation}`);

  return new Agent({
    id: `fo-14-affirmation-${implementation}`,
    name: 'FO-14 Affirmation',
