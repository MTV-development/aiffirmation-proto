...
export const fo13DiscoveryAgent = new Agent({
  id: 'fo-13-discovery',
  name: 'FO-13 Discovery',
  instructions: DEFAULT_INSTRUCTIONS,
  model: getModel('openai/gpt-4o'),
});

export async function createFO13DiscoveryAgent(implementation: string = 'default'): Promise<Agent> {
  const systemPrompt = await getAgentSystemPrompt('fo-13', implementation);
  const modelName = await getAgentModelName('fo-13', implementation);

  return new Agent({
    id: `fo-13-discovery-${implementation}`,
    name: 'FO-13 Discovery',
