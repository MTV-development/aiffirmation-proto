...
export const fo13AffirmationAgent = new Agent({
  id: 'fo-13-affirmation',
  name: 'FO-13 Affirmation',
  instructions: DEFAULT_INSTRUCTIONS,
  model: getModel(),
});

export async function createFO13AffirmationAgent(implementation: string = 'default'): Promise<Agent> {
  const systemPrompt = await getKVText(`versions.fo-13.system_affirmation.${implementation}`);
  const modelName = await getKVText(`versions.fo-13._model_name_affirmation.${implementation}`);

  return new Agent({
    id: `fo-13-affirmation-${implementation}`,
    name: 'FO-13 Affirmation',
