...
export const fo07PreSummaryAgent = new Agent({
  id: 'fo-07-pre-summary',
  name: 'FO-07 Pre-Affirmation Summary',
  instructions: PRE_AFFIRMATION_INSTRUCTIONS,
  model: getModel(),
});

// Post-affirmation agent (after generation)
export const fo07PostSummaryAgent = new Agent({
  id: 'fo-07-post-summary',
  name: 'FO-07 Post-Affirmation Summary',
  instructions: POST_AFFIRMATION_INSTRUCTIONS,
  model: getModel(),
});

export async function createFO07PreSummaryAgent(implementation: string = 'default'): Promise<Agent> {
  // Use fo-07-summary KV key (no separate pre/post in KV store)
  const systemPrompt = await getAgentSystemPrompt('fo-07-summary', implementation);
  const modelName = await getAgentModelName('fo-07-summary', implementation);

  return new Agent({
    id: `fo-07-pre-summary-${implementation}`,
    name: 'FO-07 Pre-Affirmation Summary',
...
export async function createFO07PostSummaryAgent(implementation: string = 'default'): Promise<Agent> {
  // Use fo-07-summary KV key (no separate pre/post in KV store)
  const systemPrompt = await getAgentSystemPrompt('fo-07-summary', implementation);
  const modelName = await getAgentModelName('fo-07-summary', implementation);

  return new Agent({
    id: `fo-07-post-summary-${implementation}`,
    name: 'FO-07 Post-Affirmation Summary',
...
export const fo07SummaryAgent = fo07PostSummaryAgent;
export const createFO07SummaryAgent = createFO07PostSummaryAgent;
