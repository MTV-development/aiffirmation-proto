...
export const fo12AffirmationAgent = new Agent({
  id: 'fo-12-affirmation',
  name: 'FO-12 Affirmation',
  instructions: DEFAULT_INSTRUCTIONS,
  model: getModel(),
});

export async function createFO12AffirmationAgent(implementation: string = 'default'): Promise<Agent> {
  const systemPrompt = await getKVText(`versions.fo-12.system_affirmation.${implementation}`);
  const modelName = await getKVText(`versions.fo-12._model_name_affirmation.${implementation}`);

  return new Agent({
    id: `fo-12-affirmation-${implementation}`,
    name: 'FO-12 Affirmation',
