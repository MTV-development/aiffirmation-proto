...
export const fo08AffirmationAgent = new Agent({
  id: 'fo-08-affirmation',
  name: 'FO-08 Affirmation',
  instructions: DEFAULT_INSTRUCTIONS,
  model: getModel(),
});

export async function createFO08AffirmationAgent(implementation: string = 'default'): Promise<Agent> {
  const systemPrompt = await getAgentSystemPrompt('fo-08-affirmation', implementation);
  const modelName = await getAgentModelName('fo-08-affirmation', implementation);

  return new Agent({
    id: `fo-08-affirmation-${implementation}`,
    name: 'FO-08 Affirmation',
