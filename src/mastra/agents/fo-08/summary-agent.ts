...
export const fo08PreSummaryAgent = new Agent({
  id: 'fo-08-pre-summary',
  name: 'FO-08 Pre-Affirmation Summary',
  instructions: PRE_AFFIRMATION_INSTRUCTIONS,
  model: getModel(),
});

// Post-affirmation agent (after generation)
export const fo08PostSummaryAgent = new Agent({
  id: 'fo-08-post-summary',
  name: 'FO-08 Post-Affirmation Summary',
  instructions: POST_AFFIRMATION_INSTRUCTIONS,
  model: getModel(),
});

export async function createFO08PreSummaryAgent(implementation: string = 'default'): Promise<Agent> {
  // Use fo-08-summary KV key (no separate pre/post in KV store)
  const systemPrompt = await getAgentSystemPrompt('fo-08-summary', implementation);
  const modelName = await getAgentModelName('fo-08-summary', implementation);

  return new Agent({
    id: `fo-08-pre-summary-${implementation}`,
    name: 'FO-08 Pre-Affirmation Summary',
...
export async function createFO08PostSummaryAgent(implementation: string = 'default'): Promise<Agent> {
  // Use fo-08-summary KV key (no separate pre/post in KV store)
  const systemPrompt = await getAgentSystemPrompt('fo-08-summary', implementation);
  const modelName = await getAgentModelName('fo-08-summary', implementation);

  return new Agent({
    id: `fo-08-post-summary-${implementation}`,
    name: 'FO-08 Post-Affirmation Summary',
...
export const fo08SummaryAgent = fo08PostSummaryAgent;
export const createFO08SummaryAgent = createFO08PostSummaryAgent;
