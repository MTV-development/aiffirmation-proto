...
export const fo04AffirmationAgent = new Agent({
  id: 'fo-04-affirmation-agent',
  name: 'FO-04 Affirmation',
  instructions: DEFAULT_INSTRUCTIONS,
  model: getModel(),
});

export async function createFO04AffirmationAgent(implementation: string = 'default'): Promise<Agent> {
  const systemPrompt = await getAgentSystemPrompt('fo-04-affirmation', implementation);
  const modelName = await getAgentModelName('fo-04-affirmation', implementation);

  return new Agent({
    id: `fo-04-affirmation-agent-${implementation}`,
    name: 'FO-04 Affirmation',
