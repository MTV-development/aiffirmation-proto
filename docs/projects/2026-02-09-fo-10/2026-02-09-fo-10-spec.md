...
**Input:** Step number, full conversation history (questions and answers).
**Output:** `{ initialChips: string[], expandedChips: string[] }`

**Context Format (Conversational):**

The agent receives context as a natural conversation flow, not structured extraction:

```
## Conversation So Far
Q: What is your primary goal with affirmations today, Morten?
A: Confidence Calm

Q: Why does this goal feel important to you?
A: I've been holding back because I'm afraid

## Next Question
In which situation is your goal especially important?

## Your Task
Generate 8 initial + 15 expanded complete sentences that help Morten answer THIS question based on what they've shared...
```

**Step-specific user prompt templates:**
- **Step 5 template:** Shows step 4 Q&A, then asks for 8 + 15 hybrid fragments (end with "...") to help answer "Why does this goal feel important?"
- **Step 6 template:** Shows steps 4-5 Q&A, then asks for 8 + 15 complete sentences to help answer "In which situation is your goal especially important?"
- **Step 7 template:** Shows steps 4-6 Q&A, then asks for 8 + 15 complete sentences to help answer "What kind of support would be most helpful?"

**Key Principle:** The agent sees the raw conversation (Q: ... A: ...) rather than structured labels like "Step 4 - Goal:". This makes the context more natural and easier for the LLM to understand.

**System prompt:** Focused guidance on generating contextual chips. Much simpler than FO-09's discovery agent — no 5 Dimensions framework, no question generation, no readyForAffirmations logic. Just: "Given this conversation, generate relevant response options for the next question."
